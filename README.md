...
2. **工具规划**: 人格通过模型原生 `tool_calls` 调用 `web_search_tavily`、`skill_run`、`mcp_call`，宿主映射为 `web.search.tavily`、`skill.run`、`mcp.call`；读类低风险动作可直接执行，高风险动作进入审批队列
...
- `magi:smoke:full`: 在 smoke check 后跑一轮真实 MAGI prompt，并断言 `mcp.call`、`council-tools`、`synthesis-tools`、`synthesis-stream`、auditRef、无人格掉线、无 failed events 和最终回答里的 `MAGI_CAN_SEE_CODE_YES`。
- `magi:cli`: 运行单条 prompt；支持 `--format json|jsonl`、`--stream`、`--out <path>`、`--expect-tool <id>`、`--expect-phase <phase>`、`--expect-text <text>`、`--expect-audit`、`--expect-no-offline`、`--expect-no-failed-events`、`--expect-pending`、`--save-session`。
